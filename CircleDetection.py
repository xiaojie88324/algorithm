...
def getSpecificAreaConnects(originImage, threshold=[0.15, 0.85]):
    """
    ...
    topEndPos = int(H * threshold[0])
    buttonStartPos = int(H * threshold[1])
    topImageArea = originImage[0:topEndPos, :]
    buttonImageArea = originImage[buttonStartPos:H, :]

    return topImageArea, buttonImageArea
...
if __name__ == "__main__":
    ...
    directory_name = "D:\program\code\cv\VIN\LinShi\data\data4"  # 要读取的文件夹名
    ...
    for count in range(0, len(imgList)):
        ...
        # 1. 数据预处理
        topImageArea, buttonImageArea = getSpecificAreaConnects(originImage)
        centerTop = adsa(topImageArea, filename, 0)
        centerButton = adsa(buttonImageArea, filename, 2)
        """
        binaryImage, bin01Image = dataPreprocess(originImage)
        ...
        print(f"顶部圆心：{centerTop}   底部圆心：{centerButton}")
        # 此处应该再加上圆心数量判断
        if len(centerTop) + len(centerButton) < 4:
            continue
        centers = [centerTop[0], centerTop[1], centerButton[0], centerButton[1]]

        print(centers)
        # 8. 透视变换+resize
        targetImg = Perspective_transform(originImage, np.array(centers))
        targetImg = cv2.resize(targetImg, (110, 470))
        # 9. 结果图保存
        connectSavePath = r'D:\program\code\cv\VIN\LinShi\result\result4\\' + filename
